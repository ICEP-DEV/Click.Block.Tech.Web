  ...
  useEffect(() => {
    const fetchActiveAccounts = async () => {
      try {
        ...
        const response = await axios.get(`${BASE_URL}/accounts/filter?status=active`, {
          headers: {
            Authorization: `Bearer ${token}`, // Include token if your API requires authentication
          },
        });
        setActiveAccountsData(response.data); // Update state with fetched data
        setLoading(false); // Set loading to false after data is fetched
      } catch (error) {
        console.error('Error fetching active accounts:', error.response ? error.response.data : error);
        setError('Failed to load active accounts.'); // Update error state
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="dashboard-container">
        ...
        <h2 className="dashboard-heading">ACTIVE ACCOUNTS</h2>
        <div>Loading active accounts...</div>
      </div>
    );
  }
  ...
  return (
    <div className="dashboard-container">
      ...
      <table className="data-table">
        ...
        <tbody>
          {activeAccountsData.map((account, index) => (
            <tr key={index}>
